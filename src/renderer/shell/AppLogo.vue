 ...
 *
 * This is the adaptive, in-app rendering of the same two paths that
 * `scripts/make-icons.mjs` rasterises for the packaged app icon — the design
 * source is `build/oscine-logo.svg`. Rebuilt as inline SVG here, rather than
 * referenced as a file, so it themes with everything else: the disc takes
 * `--ui-primary` and the wave `--ui-text-inverted`, the same pair the title
 * bar's badge wore, so a theme swap recolours the logo without touching this
 * file.
 *
 * Size it from the outside — `<AppLogo class="size-5" />`. The viewBox does the
 * scaling; the disc fills the square, so the mark is a full circle at any size.
 *
 * The disc geometry is normalised to plain `cx/cy/r`; the wave keeps its native
 * path under the one translate the drawing was authored with, which is cheaper
 * and safer than rebaking every coordinate by hand.
 */
</script>
