...
/**
 * The transport settings that outlive a session.
 *
 * Shuffle, repeat and volume are modes rather than actions — a player that
 * forgot them on restart would be one the user has to re-arm every launch.
 * The shuffle *seed* is deliberately not among them: design §5 rule 5 keeps
 * traversal transient in v1, so switching shuffle on after a restart reshuffles
 * rather than resurrecting last week's sequence, which is also the only honest
 * thing to do when the library may have been rescanned in between.
 *
 ...
 *
 * What is left of this module is key names and the binding the controller
...
export const TRANSPORT_VOLUME_KEY = 'playback.volume'
...
/** The bindings, writable: assigning one persists it. */
...
export function defaultTransportPreferences(): TransportPreferences {
  return {
    ...
    volume: settingDefault<number>(TRANSPORT_VOLUME_KEY)
  }
}

/**
 * Bind the transport to its persisted keys, or to nothing.
 *
...
export function bindTransportPreferences(settings?: SettingsReader): TransportBinding {
  if (!settings) {
    ...
    return {
      repeat: ref(defaults.repeat),
      shuffle: ref(defaults.shuffle),
      volume: ref(defaults.volume)
    }
  }
  return {
    repeat: settings.value<RepeatMode>(TRANSPORT_REPEAT_KEY),
    shuffle: settings.value<boolean>(TRANSPORT_SHUFFLE_KEY),
    volume: settings.value<number>(TRANSPORT_VOLUME_KEY)
  }
}
