...
/**
 * Album grouping, from the toolbar.
 *
...
const groupable = computed(() => panel.sort === 'album')
</script>
  ...
  <UPopover :ui="{ content: 'w-72' }">
    ...
    <template #content>
      <div class="flex flex-col">
        ...
        <p v-if="!groupable" class="border-t border-default px-3 py-2 text-xs text-dimmed">
          Albums are grouped when the list is sorted by Album — selecting an artist does that.
          Sorted by another column, the albums interleave and there are no runs to head.
        </p>
      </div>
    </template>
  </UPopover>
