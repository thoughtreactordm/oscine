...
electron-builder reads it as a build resource rather than a build step. It reproduces the
`AppTitleBar` mark — the tabler `wave-sine` glyph on a `bg-primary` badge — from the same two
inputs the component uses, so the app icon cannot drift from the one in the title bar. Each size is
rendered from its own SVG with a per-size stroke weight rather than downscaled from one master: the
2 px stroke that is correct at 512 px is a third of a pixel at 16 px, and no amount of resampling
recovers it. Change the mark in the script, not in the PNGs.
