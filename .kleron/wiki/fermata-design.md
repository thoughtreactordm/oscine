...
*Amended 2026-08-03 (W10-13)*: the **listens log is carried; the trail still is not**. The trigger above has fired — D17 made `tracks.play_count` and `tracks.last_played_at` maintained caches of the `listens` log rather than counters in their own right — and the amendment that set it already named the answer: carrying a derived value while dropping its source is the incoherent option. The log is also mergeable in the way the trail is not. A trail is a bounded window whose merge discards rows by an accident of ordering rather than by age; a listens log is an unbounded set of timestamped events, and two machines' events genuinely interleave into a chronology that did happen — you listened on the laptop at nine and on the desktop at three. Import is `INSERT OR IGNORE` against `UNIQUE(started_at, title, artist_name)`, `listen_genres` rides along with its parent rows, and `play_count` is **recomputed** from the merged log afterwards rather than added — adding two machines' counters after also merging their logs double-counts every overlapping listen. Snapshot columns (§4) make a listen portable on its own, so one whose track this machine does not have still imports, unlinked and intact. `track_favorites` joins the bundle on the same footing as ratings: a statement about a track, resolving by recency. Still excluded, and each for its own reason: `play_history`, unchanged and for the reason above — **do not read its new neighbour as an argument**; `scrobble_queue`, machine-local outbound state whose import would submit another machine's pending scrobbles a second time under whichever account this one is signed into; `track_genres`, derived from `tracks.genre` and rebuilt on scan; and any credential at all — the Last.fm session key is in `safeStorage` precisely so that it cannot be here (D19). Which side **every** table is on is now declared once, in `src/main/bundle/tables.ts`, and asserted against the live schema, so a migration cannot add a table the bundle quietly decides about; tables nobody has ruled on are declared `open` rather than guessed at.
